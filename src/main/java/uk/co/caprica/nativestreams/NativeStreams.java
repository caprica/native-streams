...
package uk.co.caprica.nativestreams;

import com.sun.jna.Pointer;
import uk.co.caprica.nativestreams.binding.LibC;
...
public final class NativeStreams {
    ...
    private boolean redirectOutputTo(String target) {
        outputStream = LibC.fdopen(STDOUT_FD, STREAM_MODE);
        if (outputStream != null) {
            redirectedOutputStream = LibC.freopen(target, STREAM_MODE, outputStream);
    ...
    private boolean redirectErrorTo(String target) {
        errorStream = LibC.fdopen(STDERR_FD, STREAM_MODE);
        if (errorStream != null) {
            redirectedErrorStream = LibC.freopen(target, STREAM_MODE, errorStream);
    ...
    public void release() {
        if (redirectedOutputStream != null) {
            LibC.fclose(redirectedOutputStream);
        }
        if (redirectedErrorStream != null) {
            LibC.fclose(redirectedErrorStream);
        }
    }
}
